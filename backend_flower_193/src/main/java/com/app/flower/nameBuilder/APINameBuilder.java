package com.app.flower.nameBuilder;
...
public class APINameBuilder implements JPAEdmNameBuilder {
	...
	public String buildEntitySetName(final String entityTypeName) {
    	...
    	eSetName = "Department".equals(entityTypeName) ? "Departments" : defaultNameBuilder.buildEntitySetName(entityTypeName);
    	eSetName = "College".equals(entityTypeName) ? "Colleges" : defaultNameBuilder.buildEntitySetName(entityTypeName);
    	eSetName = "PreviousEducation".equals(entityTypeName) ? "PreviousEducations" : defaultNameBuilder.buildEntitySetName(entityTypeName);
    	eSetName = "Student".equals(entityTypeName) ? "Students" : defaultNameBuilder.buildEntitySetName(entityTypeName);
    	eSetName = "Course".equals(entityTypeName) ? "Courses" : defaultNameBuilder.buildEntitySetName(entityTypeName);
    	eSetName = "Lecturer".equals(entityTypeName) ? "Lecturers" : defaultNameBuilder.buildEntitySetName(entityTypeName);
    	eSetName = "Subject".equals(entityTypeName) ? "Subjects" : defaultNameBuilder.buildEntitySetName(entityTypeName);
        return eSetName;
    }
